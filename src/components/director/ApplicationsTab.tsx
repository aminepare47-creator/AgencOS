...
export const ApplicationsTab: React.FC<ApplicationsTabProps> = ({ onSelectCandidature }) => {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-[#D9E2EC] shadow-xs space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t border-[#D9E2EC]">
          ...
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-bold transition-colors font-mono cursor-pointer ${
              filterStatut === 'all' 
                ? 'bg-[#14456F] text-white' 
                : 'bg-[#F7F9FC] text-[#545f73] border border-[#C3CEDA] hover:text-[#0B2A4A]'
            }`}
          >
      ...
      <div className="bg-white rounded-2xl border border-[#D9E2EC] shadow-xs overflow-hidden">
        {filteredCandidatures.length === 0 ? (
          <div className="py-12 text-center text-[#545f73] text-xs">
            ...
            <p className="font-bold text-[#0B2A4A]">Aucun dossier ne correspond à vos filtres.</p>
            ...
            {filteredCandidatures.map(cand => {
              ...
              return (
                <div
                  ...
                  className={`p-4 sm:p-5 hover:bg-[#F7F9FC] transition-colors cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
                    isToReview ? 'bg-amber-50/50' : ''
                  }`}
                >
                  <div className="flex items-start gap-3.5">
                    <div className="w-10 h-10 rounded-xl bg-[#F7F9FC] border border-[#C3CEDA] text-[#0B2A4A] flex items-center justify-center font-bold text-sm shrink-0 mt-0.5 font-mono">
                      {cand.candidat.nom.slice(0, 2).toUpperCase()}
                    </div>

                    <div>
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className="font-mono text-xs font-bold text-[#0B2A4A] bg-[#E1EAF2] border border-[#14456F]/20 px-2 py-0.5 rounded">
                          {cand.codeSuivi}
                        </span>
                        <StatusBadge statut={cand.statut} />
                        {cand.soumission?.noteDirecteur !== undefined && (
                          <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-900 bg-[#C9DFF2] border border-emerald-300 px-2 py-0.5 rounded font-mono">
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-3 pt-2 sm:pt-0 border-t sm:border-t-0 border-[#D9E2EC]">
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors cursor-pointer ${
                        isToReview
                          ? 'bg-[#14456F] text-white hover:bg-[#0B2A4A] shadow-xs'
                          : 'bg-[#F7F9FC] text-[#0B2A4A] border border-[#C3CEDA] hover:bg-[#E1EAF2]'
                      }`}
                    >
